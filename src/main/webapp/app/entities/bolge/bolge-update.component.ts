...
export class BolgeUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.departmanService
            .query()
            .pipe(
                filter((mayBeOk: HttpResponse<IDepartman[]>) => mayBeOk.ok),
                map((response: HttpResponse<IDepartman[]>) => response.body)
            )
            .subscribe((res: IDepartman[]) => (this.departmen = res), (res: HttpErrorResponse) => this.onError(res.message));
    }
