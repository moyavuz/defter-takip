...
import { IscilikDetayService } from 'app/entities/iscilik-detay';
...
export class PozUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.iscilikDetayService
            .query()
            .pipe(
                filter((mayBeOk: HttpResponse<IIscilikDetay[]>) => mayBeOk.ok),
                map((response: HttpResponse<IIscilikDetay[]>) => response.body)
            )
            .subscribe((res: IIscilikDetay[]) => (this.iscilikdetays = res), (res: HttpErrorResponse) => this.onError(res.message));
    }
    ...
    trackIscilikDetayById(index: number, item: IIscilikDetay) {
        return item.id;
    }
}
