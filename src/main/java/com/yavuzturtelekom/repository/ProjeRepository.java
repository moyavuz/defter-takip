...
public interface ProjeRepository extends JpaRepository<Proje, Long> {
    ...
    @Query(value = "select distinct proje from Proje proje left join fetch proje.isciliks",
    ...
    @Query("select proje from Proje proje left join fetch proje.isciliks where proje.id =:id")
