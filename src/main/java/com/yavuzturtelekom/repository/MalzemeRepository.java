...
public interface MalzemeRepository extends JpaRepository<Malzeme, Long> {
    ...
    @Query(value = "select distinct malzeme from Malzeme malzeme left join fetch malzeme.isciliks",
    ...
    @Query("select malzeme from Malzeme malzeme left join fetch malzeme.isciliks where malzeme.id =:id")
