...
public interface EkipRepository extends JpaRepository<Ekip, Long> {
    ...
    @Query(value = "select distinct ekip from Ekip ekip left join fetch ekip.calisans",
    ...
    @Query("select ekip from Ekip ekip left join fetch ekip.calisans where ekip.id =:id")
