...
public interface EkipRepository extends JpaRepository<Ekip, Long> {
    ...
    @Query(value = "select distinct ekip from Ekip ekip left join fetch ekip.ekipCalisanListesis",
    ...
    @Query("select ekip from Ekip ekip left join fetch ekip.ekipCalisanListesis where ekip.id =:id")
