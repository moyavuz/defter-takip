...
public class IscilikDetay implements Serializable {
    ...
    @ManyToOne
    @JsonIgnoreProperties("detays")
    private Iscilik iscilik;

    @OneToMany(mappedBy = "iscilikDetay")
    @Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
    ...
    public IscilikDetay pozs(Set<Poz> pozs) {
        this.pozs = pozs;
        return this;
    }

    public IscilikDetay addPoz(Poz poz) {
        this.pozs.add(poz);
        poz.setIscilikDetay(this);
        return this;
    }

    public IscilikDetay removePoz(Poz poz) {
        this.pozs.remove(poz);
        poz.setIscilikDetay(null);
        return this;
    }
