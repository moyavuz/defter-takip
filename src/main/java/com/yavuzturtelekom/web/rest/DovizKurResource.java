...
public class DovizKurResource {
    ...
    public ResponseEntity<List<DovizKur>> getAllDovizKurs(DovizKurCriteria criteria, Pageable pageable) {
        log.debug("REST request to get DovizKurs by criteria: {}", criteria);
        Page<DovizKur> page = dovizKurQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/doviz-kurs");
