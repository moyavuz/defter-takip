...
public class MalzemeResource {
     ...
     */
    @GetMapping("/malzemes")
    public ResponseEntity<List<Malzeme>> getAllMalzemes(Pageable pageable, @RequestParam(required = false) String filter, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        ...
        Page<Malzeme> page;
        if (eagerload) {
            page = malzemeService.findAllWithEagerRelationships(pageable);
        } else {
            page = malzemeService.findAll(pageable);
        }
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, String.format("/api/malzemes?eagerload=%b", eagerload));
