...
public class ProjeResourceIntTest {
    ...
    public static Proje createEntity(EntityManager em) {
        ...
        // Add required entity
        Iscilik iscilik = IscilikResourceIntTest.createEntity(em);
        em.persist(iscilik);
        em.flush();
        proje.getIsciliks().add(iscilik);
        return proje;
    }
    ...
    @Test
    @Transactional
    public void checkIsTanimiIsRequired() throws Exception {
        int databaseSizeBeforeTest = projeRepository.findAll().size();
        // set the field null
        proje.setIsTanimi(null);
